...
function generateCookingStyles(nbCookingStyles) {
    ...
    while (cookingStyles.size < nbCookingStyles) {
        cookingStyles.add(faker.address.country());
    }
    return [...cookingStyles].map((label) => ({ label }));
}
...
function generateCities(nbCities) {
    ...
    for (let i = 0; i < nbCities; i += 1) {
        ...
        const city = {
            ...
            geopos: [faker.address.latitude(), faker.address.longitude()],
        };
        cities.push(city);
    }
    return cities;
